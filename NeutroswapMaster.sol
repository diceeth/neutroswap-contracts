...
// File: src/nft-pool-factory/interfaces/IYieldBooster.sol
...
// File: src/nft-pool-factory/interfaces/INeutroswapMaster.sol
...
// File: src/nft-pool-factory/interfaces/tokens/INeutroTokenV2.sol
...
interface INeutroTokenV2 is IERC20 {
    function lastEmissionTime() external view returns (uint256);

    function claimMasterRewards(uint256 amount)
        external
...
// File: https://github.com/OpenZeppelin/openzeppelin-contracts/blob/solc-0.7/contracts/token/ERC20/SafeERC20.sol
...
// File: src/nft-pool-factory/interfaces/INFTPool.sol
...
// File: src/nft-pool-factory/NeutroswapMaster.sol
...
/*
 * This contract centralizes Neutroswap's yield incentives distribution.
 * Pools that should receive those incentives are defined here, along with their allocation.
 * All rewards are claimed from the NEUTROToken contract.
 */
contract NeutroswapMaster is Ownable, INeutroswapMaster {
  using SafeERC20 for INeutroTokenV2;
  using SafeMath for uint256;
  using EnumerableSet for EnumerableSet.AddressSet;
  ...
  INeutroTokenV2 private immutable _neutroToken; // Address of the NEUTRO token contract
  ...
  constructor(
    INeutroTokenV2 neutroToken_,
    uint256 startTime_
  ) {
    require(address(neutroToken_) != address(0), "NeutroswapMaster: neutroToken cannot be set to zero address");
    require(_currentBlockTimestamp() < startTime_ && startTime_ >= neutroToken_.lastEmissionTime(), "NeutroswapMaster: invalid startTime");
    _neutroToken = neutroToken_;
    startTime = startTime_; // Must be set with the same time as NeutroToken emission start
  }
  ...
  /*
   * @dev Returns NeutroToken's emission rate (allocated to this contract)
   */
  function emissionRate() public view returns (uint256) {
    return _neutroToken.masterEmissionRate();
  }

  /**
   * @dev Returns current owner's address
   */
  function owner() public view virtual override(INeutroswapMaster, Ownable) returns (address) {
  ...
  function _updatePool(address poolAddress) internal {
    ...
    if (allocPoint > 0 && INFTPool(poolAddress).hasDeposits()) {
      // calculate how much NEUTRO rewards are expected to be received for this pool
      ...
      (rewards) = _neutroToken.claimMasterRewards(rewards);
